...
class World(object):
	...
	def __init__(self, name, width, height, gridDim = 10, delay = 0.15):
		...
		self.finishFlag = None # assume there can only be one end flag
		self.goombaList = [] 
		...
		#for novelty:
		self.possibleTravel = 93 #for small world
	...
	def calculateMaxCoinScore(self):
		self.maxCoinScore = len(self.goombaList) + len(self.coinsList) + len(self.coinboxesList) + 15*len(self.hiddenRoomBonuses) + 5
...
class Mario(object):
	...
	def __init__(self, world, name, x, y, color = "red"):
		...
		self.coinType = 0 # flag turns to 1 if Mario's nearest coin is a coinbox
	...
	def checkBounds(self):
		...
		elif (self.nextX, self.nextY) == self.world.finishFlag:
	...
	def isStalled(self):
		""" stall implementation: If in the same x position for 3 timesteps, 
		   	self.stall will equal True """

		if self.x == self.lastX:
			self.stallCount += 1

		else: #changed position, supposedly not stalled now
			self.stallCount = 0
	...
	def update(self, cmd):
		...
		self.checkBounds() # check if move is valid

		self.isStalled() # check if mario has been at x pos for > 3 turns

		if (self.x, self.y) not in self.behavior: #check if novel behavior
	...
	def distanceToNearestCoin(self):
		...
		marPos = (self.x, self.y)
		factor = 1


		for coin in self.world.coinsList:
			distance = self.calculateDistance(coin, marPos)

			if distance < minDistance:
				minDistance = distance
				self.coinType = -1

				if (self.x - coin[0]) > 0: # coin is to the left of mario
					factor = -1
				else:
					factor = 1
		...
		for goomba in self.world.goombaList:
			distance = self.calculateDistance(goomba, marPos)
			
			if distance < minDistance:
				minDistance = distance
				self.coinType = -0.5

				if (self.x - goomba[0]) > 0: # coin is to the left of mario
					factor = -1
				else:
					factor = 1
		...
		for entrance in self.world.hiddenEntrances:
			distance = self.calculateDistance(entrance, marPos)
			
			if distance < minDistance:
				minDistance = distance
				self.coinType = 0

				if (self.x - entrance[0]) > 0: # coin is to the left of mario
					factor = -1
				else:
					factor = 1
		...
		for coinbox in self.world.coinboxObjectList:
			if coinbox.stillContainsCoin:
				distance = self.calculateDistance((coinbox.x, coinbox.y), marPos)

				if distance < minDistance:
					minDistance = distance
					self.coinType = 0.5

					if (self.x - coinbox.x) > 0: # coin is to the left of mario
						factor = -1
				else:
					factor = 1

		# flag
		flagDistance = self.calculateDistance((self.world.finishFlag[0], self.world.finishFlag[1]), marPos)
		if flagDistance < minDistance:
			minDistance = distance
			self.coinType = 1

			if (self.x - self.world.finishFlag[0]) > 0: # coin is to the left of mario
				factor = -1
			else:
				factor = 1
