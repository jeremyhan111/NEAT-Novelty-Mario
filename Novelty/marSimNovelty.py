...
class World(object):
		...
		for i in range(self.numGridX):
			for j in range(self.numGridY):
				...
				if potentialAirspace in self.ground:
					...
					continue
				elif potentialAirspace in self.validStand:
					continue
	...
	def calculateMaxCoinScore(self):
		maxCoinScore = len(self.goombas) + len(self.coinsList) + len(self.coinboxesList)
	...
	def worldDetailsForNoveltyHelper(self):
		...
		totalGridSpots = self.numGridX * self.numGridY
		totalPossibleTravelLocations = totalGridSpots - len(self.coinboxesList) - len(self.ground)

		print "totalGridSpots: ", totalGridSpots, "totalPossibleTravelLocations", totalPossibleTravelLocations

		print "valid stand: ", len(self.validStand), "validAirspace", len(self.validAirspace)
		calcTotal = len(self.validStand) + len(self.validAirspace)

		print "calcTotal? ", len(self.validStand) + len(self.validAirspace)

		print "after sub: ", calcTotal - 49
...
class Mario(object):
	...
	def fall(self):
		""" Mario is in the air and is falling """
		self.nextX = self.x + 0
		self.nextY = self.y + 1
	...
	def checkBounds(self):
		...
		# handles translate?
			...
			if self.duck and (self.x, self.y) in self.world.hiddenEntrances:
				...
				print "got hidden room bonus of:", self.world.hiddenRoomBonuses[bonusIndex]
	...
	def update(self, cmd):
		...
		# print "mario is now at: ", (self.x, self.y), "\n"
